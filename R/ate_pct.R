...
#'
#' @examples
#' ## Suppose variables gr1, gr2, gr3 are indicators for three sub-treatment groups.
#' ## We are interested in the ATE in percentage points of these three treatment groups.
#' ## First run a semi-log regression with subgroup-specific treatment effects.
#' ## (y must be log-transformed)
#' data(ate_pct_example, package = "atepct")
#' ## Run the regression
#' reg_res <- lm(lny~x+gr1+gr2+gr3,data=ate_pct_example)
#'
#' ## Example 1: ATE in percentage points for three groups (weights by sample size)
#' out1 <- ate_pct(reg_res, c("gr1","gr2","gr3"))
#' out1
#' summary(out1)
#'
#' ## By default, ate_pct() uses vcov(reg_res) as the covariance matrix of tauhat.
#' ## If you want heteroskedasticity-robust SEs for tauhat, use sandwich (if available):
...
#'
#' ## Example 2: weights are treated as fixed (no sampling variation).
#' ## Also, use the conventional VC matrix for log points estimator.
#' out2 <- ate_pct(reg_res, c("gr1","gr2","gr3"), truew=TRUE)
#'
#' ## Example 3: ATE in percentage points for the first two groups,
#' ## and assign equal weights (so weight are fixed and known.)
#' out3 <- ate_pct(reg_res, c("gr1","gr2"), truew=TRUE, groupsize=c(1,1))
#'
#' ## Example 4: Provide subgroup sizes externally.
#' ## The command below is this is exactly the same as
#' ## \code{ate_pct gr1 gr2 gr3}
#' ## as 15, 24, 37 are the sample size for the three treatment groups.
#' out4 <- ate_pct(reg_res, c("gr1","gr2","gr3"), groupsize=c(15,24,37))
#'
#' @export
#' @keywords models
